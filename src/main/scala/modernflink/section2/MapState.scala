...
object MapState:
  ...
  def mapStateDemo(): Unit =
    val humidityLevelStream: DataStream[KeyedHumidityLevel] = humidityData
      ...
      .trigger(PurgingTrigger.of(CountTrigger.of[Window](7))) // Every 7 elements, Flink will create a new Global Window, then clearing the window
          ...
          override def open(parameters: Configuration): Unit = {
            humidityLevelCount = getRuntimeContext.getMapState(
              new MapStateDescriptor[HumidityLevel, Long](
                "humidityLevelCounter", classOf[HumidityLevel], classOf[Long]
              )
            )
          }

          override def processElement(value: KeyedHumidityLevel, ctx: KeyedProcessFunction[String, KeyedHumidityLevel, String]#Context, out: Collector[String]): Unit = {
            ...
            out.collect(s"${ctx.getCurrentKey} - ${humidityLevelCount.entries().asScala.mkString(",")}")
          }
        }
      )
