...
object BankingEventGenerator {
  ...
  class DepositEventGenerator(sleepSeconds: Int, startTime: Instant = Instant.now()
                             ) extends SourceFunction[Deposit] {

    import DepositEventGenerator._

    private var running = true
    private var maxEvents = 30
    ...
    private def emitEvents(num: Long) = {
      Deposit(genUser, startTime.plusSeconds(num), scala.util.Random.nextInt(100000), genCurrency)
    }
    override def run(
